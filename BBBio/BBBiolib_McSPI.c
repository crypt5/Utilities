...
const unsigned int McSPI_AddressOffset[] =
		{ MCSPI0_MMAP_ADDR, MCSPI1_MMAP_ADDR };
...
/* ----------------------------------------------------------------------------------------------- */
static inline void write_reg(volatile void *reg_base, unsigned int offset,
		unsigned int data) {
	*((volatile unsigned int*) (reg_base + offset)) = data;
}
/* ----------------------------------------------------------------------------------------------- */
static inline unsigned int read_reg(volatile void *reg_base,
		unsigned int offset) {
...
int BBBIO_McSPI_Init() {
	...
	for (i = 0; i < MCSPI_ARG_MODULE_COUNT; i++) {
		mcspi_ptr[i] = mmap(0, MCSPI_MMAP_LEN, PROT_READ | PROT_WRITE,
				MAP_SHARED, memh, McSPI_AddressOffset[i]);
 ...
 */
int BBBIO_McSPI_work(unsigned int SPI_ID, unsigned int chn,
		unsigned int Tx_data, unsigned int *Rx_data) {
	unsigned int chn_offset = 0;
	unsigned int reg_value;
	//unsigned int Tx_flag =0; Unused
	...
	if (McSPI_Module[SPI_ID].CM_PER_enable) {
		if (McSPI_Module[SPI_ID].CH[chn].status & McSPI_CH_ENABLE) {
			...
			if (chn_ptr->flag & McSPI_TxRx_Tx) {
				chn_ptr->Tx = Tx_data;
				write_reg(mcspi_ptr[SPI_ID], MCSPI_TX0 + chn_offset,
						chn_ptr->Tx);
			...
			if (chn_ptr->flag & McSPI_TxRx_Tx) {
				while (!MCSPI_GET_CHSTAT_EOT(reg_value)) {
					reg_value = read_reg(mcspi_ptr[SPI_ID],
							MCSPI_CH0STAT + chn_offset);
					sched_yield();
				}
			}
			/* waiting for RXS */
			if (chn_ptr->flag & McSPI_TxRx_Rx) {
				...
				/* copy receive data */
				if (Rx_data != NULL) {
					chn_ptr->Rx = read_reg(mcspi_ptr[SPI_ID],
							MCSPI_RX0 + chn_offset);
					*Rx_data = chn_ptr->Rx;
				}
			}
...
unsigned int WL) /* WL 			, word length */
{
	...
	if (!McSPI_Module[SPI_ID].CM_PER_enable) {
		...
		printf(
				"BBBIO_McSPI_channel_ctrl: Sorry , BBBio only for master mode .\n");
		param_error = 1;
	} else if (WL < 4 || WL > 32)
		param_error = 1;
	...
	if (!param_error) {
		...
		write_reg(mcspi_ptr[SPI_ID], MCSPI_CH0CONF + chn_offset,
				(DataDir << 16 | TRM << 12 | (WL - 1) << 7 | EPOL << 6
						| CLK_div << 2 | CLKmode));
...
int BBBIO_McSPI_CLK_set(unsigned int SPI_ID, int enable, int idle) {
	...
	if (!BBBIO_McSPI_EP_check(SPI_ID)) {
		printf(
				"BBBIO_McSPI_CLK_set: No effect pin mux Detected of SPI-%d ,Please use EP_STATUS toolkit to check Pin mux mode\n",
				SPI_ID);
		return 0;
	}
